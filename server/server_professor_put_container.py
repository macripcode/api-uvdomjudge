...
channel.queue_declare(queue='queue_put_container')


def put_container(container):
    url= 'http://127.0.0.1:8001/uv-domjudge/v1/containers/'+container["name_container"]+'/'
    response = requests.put(url, json=container)
    return response.status_code


def on_request(ch, method, props, body):
    container = json.loads(body.decode('utf-8'))
    response = put_container(container)
...
channel.basic_consume(on_request, queue='queue_put_container')
